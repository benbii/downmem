...
C=$3; CXX="$3++"; L="-DLLVM_ENABLE_LLD=ON -DLLVM_ENABLE_LTO=Thin"
rm -r build || true
if ! which "$C" "$CXX"; then
  C=gcc; CXX=g++; L=
fi
...
cmake -S ../../compiler-rt -B. -GNinja \
  ...
  "-DCOMPILER_RT_INSTALL_PATH=$1/lib/clang/22"
ninja install
mkdir -p "$1/lib/clang/22/lib/riscv32--"
cp "$1/lib/clang/22/lib/baremetal/libclang_rt.builtins-riscv32.a" \
  "$1/lib/clang/22/lib/riscv32--/libclang_rt.builtins-riscv32.a"
