...
dot_product(DTYPE *vectorA, DTYPE *vectorA_aux, DTYPE *vectorB, DTYPE *result) {
  for (uint32_t i = 0; i < BLOCK_SIZE / sizeof(DTYPE); i++) {
    for (uint32_t j = 0; j < DOTPIP; j++) {
      if ((j + i) > BLOCK_SIZE / sizeof(DTYPE) - 1) {
        result[j] +=
            vectorA_aux[(j + i) - BLOCK_SIZE / sizeof(DTYPE)] * vectorB[i];
      } else
...
int main_kernel1() {
  ...
  uintptr_t mem_offset = (uintptr_t)DPU_MRAM_HEAP_POINTER;
  // Starting address of the query subsequence
  uintptr_t current_mram_block_addr_query = (uintptr_t)(mem_offset);
  mem_offset += query_length * sizeof(DTYPE);
  // Starting address of the time series slice
  mem_offset += myStartElem * sizeof(DTYPE);
  uintptr_t starting_offset_ts = mem_offset;
  uintptr_t current_mram_block_addr_TS = (uintptr_t)mem_offset;
  // Starting address of the time series means
  mem_offset += (slice_per_dpu + query_length) * sizeof(DTYPE);
  uintptr_t current_mram_block_addr_TSMean = (uintptr_t)(mem_offset);
  // Starting address of the time series standard deviations
  mem_offset += (slice_per_dpu + query_length) * sizeof(DTYPE);
  uintptr_t current_mram_block_addr_TSSigma = (uintptr_t)(mem_offset);
  ...
  for (uint32_t i = myStartElem; i < myEndElem; i += (BLOCK_SIZE / sizeof(DTYPE))) {
    ...
    current_mram_block_addr_TS =
        (uintptr_t)starting_offset_ts + (i - myStartElem) * sizeof(DTYPE);
    current_mram_block_addr_query = (uintptr_t)DPU_MRAM_HEAP_POINTER;

    for (uint32_t j = 0; j < (query_length) / (BLOCK_SIZE / sizeof(DTYPE));
         j++) {
      mram_read((__mram_ptr void const *)current_mram_block_addr_TS, cache_TS,
                BLOCK_SIZE);
      mram_read((__mram_ptr void const *)current_mram_block_addr_TS +
                    BLOCK_SIZE,
                cache_TS_aux, BLOCK_SIZE);
      mram_read((__mram_ptr void const *)current_mram_block_addr_query,
                cache_query, BLOCK_SIZE);
    ...
    mram_read((__mram_ptr void const *)current_mram_block_addr_TSMean,
              cache_TSMean, BLOCK_SIZE);
    mram_read((__mram_ptr void const *)current_mram_block_addr_TSSigma,
    ...
    for (uint32_t k = 0; k < (BLOCK_SIZE / sizeof(DTYPE)); k++) {
      distance = 2 * ((DTYPE)query_length -
                      (cache_dotprods[k] -
                       (DTYPE)query_length * cache_TSMean[k] * query_mean) /
                          (cache_TSSigma[k] * query_std));
