...
int main() {
  ...
  if (rest_rows > 0) {
    if ((tasklet_id + tasklet_id) >= rest_rows) {
      ...
      if ((rest_rows & 1) == 1)
        start_row = (hlf_rest_rows + 1) * (dbl_chunks + 2) +
                    (tasklet_id - 1 - hlf_rest_rows) * dbl_chunks;
      else
        start_row = (hlf_rest_rows) * (dbl_chunks + 2) +
  ...
  uintptr_t mram_base_addr_A =
      (uintptr_t)(DPU_MRAM_HEAP_POINTER + start_row * n_size * sizeof(T));
  uintptr_t mram_base_addr_B =
      (uintptr_t)(DPU_MRAM_HEAP_POINTER + max_rows * n_size_pad * sizeof(T));
  uintptr_t mram_base_addr_C =
      (uintptr_t)(DPU_MRAM_HEAP_POINTER + max_rows * n_size_pad * sizeof(T) +
                 n_size_pad * sizeof(T) + start_row * sizeof(T));
  uintptr_t mram_temp_addr_A = mram_base_addr_A;
  uintptr_t mram_temp_addr_B = mram_base_addr_B;
  ...
  for (unsigned int i = start_row; i < start_row + rows_per_tasklet; i += 2) {
    mram_temp_addr_A =
        (uintptr_t)(DPU_MRAM_HEAP_POINTER + i * n_size * sizeof(T));
