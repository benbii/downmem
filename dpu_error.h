...
typedef enum dpu_error_t {
  ...
  /** Error returned when trying to execute a parallel memory transfer on
     MRAM/IRAM. */
   ...
   * Unlike the other errors, this one is a bitfield, and can be combined with
   * other errors. Cast to an int because enums have signed integer range, and
   * we want to be able to use the bit 31. */
 ...
 * @return The string associated to the specified status. The user is
 * responsible to free the returned string when it is not needed anymore.
 */
