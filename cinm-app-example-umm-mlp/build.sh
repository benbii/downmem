...
DPU_SRC="devApp/mlp.dpu.c"

# 修改 mlp.dpu.c 文件（如果需要）
# 注意：mlp.dpu.c 的结构与 mv.dpu.c 类似，每个 DPU 处理一个矩阵行
# mm_dimm1_nopt 使用步长 4096，这对于单 DPU 应该是可以的
# 如果需要多 tasklet 支持，可能需要调整步长，但这里先保持原样
...
# 使用mm_dimm1_nopt版本（对应1个tasklet）
source "$U"
dpu-upmem-dpurte-clang -O2 devApp/$S.dpu.c -DNR_TASKLETS=$T -DCOMPILE_mm_dimm1_nopt -o devApp/bins/$S.ummbin
...
source setup_env.sh
./build/mlp 1 1024 devApp/objdumps/mlp.objdump
