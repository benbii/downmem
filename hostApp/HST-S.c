...
int main(int argc, char** argv) {
  ...
  int nr_of_dpus = atoi(argv[2]), input_size = atoi(argv[1]);
  ...
  int bins = 256;
  int *A = malloc(input_size_dpu_8bytes * nr_of_dpus * sizeof(int));
  int *bufferA = A;
  int *histo_host = malloc(bins * sizeof(int));
  int *histo = malloc(nr_of_dpus * bins * sizeof(int));
  for (int i = 0; i < input_size; i++) { A[i] = i % 4096; }
  for (int i = 0; i < bins; i++) { histo_host[i] = 0; }
  for (int i = 0; i < nr_of_dpus * bins; i++) { histo[i] = 0; }
  ...
  input_arguments[nr_of_dpus - 1].size =
      (input_size_8bytes - input_size_dpu_8bytes * (nr_of_dpus - 1)) * sizeof(int);
  ...
  dpu_push_xfer(dpu_set, DPU_XFER_FROM_DPU, DPU_MRAM_HEAP_POINTER_NAME,
                input_size_dpu_8bytes * sizeof(int), bins * sizeof(int),
                DPU_XFER_DEFAULT);
